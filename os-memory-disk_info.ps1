######################################################################################
#　OSとメモリーと内蔵ディスクとIPアドレスの情報を収集しファイル出力するPowerShellスクリプト
######################################################################################
# ・特にPCを購入時に製品スペックや温度で故障状態を知りたくなるが調べるのが、これなら簡単と思い作成した。
# ・PCのスペックや状態を記録したファイルの出力日時をファイル名にすることで、ファイルの上書き紛失も防げるようにした。
# ・2024年9月7日にAIを使い企画は私がプロンプトし、AIがプログラミングのみし、30分ほどで完成した。
# ・Win32なのでWindowsのみで可動か？は、PowerShellCoreのマルチOS対応からして不明。
# ・Domainは特に個人利用の場合はワークグループの場合もある。
...
### 以下のヘッダーは本スクリプト以外の多くのPowerShellスクリプトでも使った方が良い機能 ###
# AIが以下のコメントを「任意のキー入力待ち」と変えてしまう、CUIではなく簡単にクリックで起動を人間が設計した
